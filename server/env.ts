const DEFAULT_SESSION_SECRET = "tobseytech-secret-key";

export const isProduction = process.env.NODE_ENV === "production";
export const MONGODB_URI = process.env.MONGODB_URI?.trim();
export const SESSION_SECRET = process.env.SESSION_SECRET?.trim() || DEFAULT_SESSION_SECRET;
...
export function validateRuntimeEnv(): void {
  ...
  if (!process.env.SESSION_SECRET?.trim()) {
    const message =
      "SESSION_SECRET is not configured. Development will use a local default; production must set a long random secret.";
    if (isProduction) {
      errors.push(message);
    } else {
      warnings.push(message);
